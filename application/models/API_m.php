...
class API_m extends CI_Model {
    ...
    public function GetCnicRecord($table_name, $table_field_name_with_value) {
        $result = $this->db->where($table_field_name_with_value)->get($table_name)->row();
        if (!$result) {
            return 0;
        } else {
            return $result;
      ...
      public function SingleTestRecord($id){
            ...
            $query1  =  $this->db->select("*")
                    ...
                    } else if($query1->testHelp_id==6)
                    {
                        ...
                        $res['testType']    = $this->db->select("*")
                                                        ->from("mrt")
                                                        ->where("mrt.testDetails_id",$query1->testDetails_id)
                                                        ->get()->row();
                    } else if($query1->testHelp_id==7)
                    {
                        ...
                        $res['testType']    = $this->db->select("*")
                                                        ->from("rbpt")
                                                        ->where("rbpt.testDetails_id",$query1->testDetails_id)
                                                        ->get()->row();
                    } else if($query1->testHelp_id==8)
                    {
                        ...
                        $res['testType']    = $this->db->select("*")
                                                        ->from("spat_human")
                                                        ->where("spat_human.testDetails_id",$query1->testDetails_id)
                                                        ->get()->row();
                    }else if($query1->testHelp_id==9)
                    {
                        ...
                        $res['testType']    = $this->db->select("*")
                                                        ->from("tuberculin_skin_test")
                                                        ->where("tuberculin_skin_test.testDetails_id",$query1->testDetails_id)
                                                        ->get()->row();
                    }else if($query1->testHelp_id==12)
                    {
                        $res['testDetails'] = $query1;
                        $res['testType']    = $this->db->select("*")
                                                                    ->from("elisa_human")
                                                                    ->where("elisa_human.testDetails_id",$query1->testDetails_id)
                                                                    ->get()->row();
                    }else if($query1->testHelp_id==13)
                    {
                        $res['testDetails'] = $query1;
                        $res['testType']    = $this->db->select("*")
                                                                    ->from("elisa_animal")
                                                                    ->where("elisa_animal.testDetails_id",$query1->testDetails_id)
                                                                    ->get()->row();
                    }else if($query1->testHelp_id==14)
                    {
                        $res['testDetails'] = $query1;
                        $res['testType']    = $this->db->select("*")
                                                                    ->from("pcr_human")
                                                                    ->where("pcr_human.testDetails_id",$query1->testDetails_id)
                                                                    ->get()->row();
                    }else if($query1->testHelp_id==15)
                    {
                        $res['testDetails'] = $query1;
                        $res['testType']    = $this->db->select("*")
                                                                    ->from("pcr_animal")
                                                                    ->where("pcr_animal.testDetails_id",$query1->testDetails_id)
                                                                    ->get()->row();
                    }
        ...
        public function SingleTestdetailsRecord($id){
            ...
            $query1  =  $this->db->select("*")
                                           ->from("testdetails")
                                            ...
                                            ->join("districts",'districts.district_id=client_info.district_id','left')
                                            ->join("tehsil",'tehsil.tehsil_id=client_info.tehsil_id','left')
                                            ->join("cattles",'cattles.cattle_id=testdetails.cattle_name','left','left')
                                            ->join("breeds",'breeds.breed_id=testdetails.cattle_breed','left')
                                            ->join("tests",'tests.test_id=testdetails.test_id')
                                            ->join('testhelp as th','th.testHelp_id=tests.testHelp_id')
                                            ->join("samples",'samples.sample_id=testdetails.sample_id','left')
                                            // ->join("cattles",'cattles.cattle_id=testdetails.cattle_name','left','left')
                                            ->where("testdetails.testDetails_id",$id)
                                            ->get()->row();
            // echo "<pre>";
        ...
        public function countByLabTestType($lab_id,$id,$status)
        {
...
//  TOTAL RECORDS  WITHOUT LAB WISE
 ...
 ***********************                                Search Page Queries                        ********************************
 **********************************************************************************************************************************/
  public function getSearchUserLabInfo($trackingID)
     {
        ...
        return $this->db->select('*')
                         ...
                         ->where('testdetails.tracking_id',$trackingID)
                         ->get()->row();
           
     }
   public function SearchResultByTrackingId($trackingID){
            $res= []; 
            $query1  =  $this->db->select("*")
                                             ->from("testdetails")
                                            ->join("client_info",'client_info.client_id=testdetails.client_id')
   ...
   public function getTestNames($lab_id) {
        return $this->db->select("*")
                                ->from("tests")
                                ->join("testhelp",'testhelp.testHelp_id=tests.testHelp_id','left')
                                ->where("tests.lab_id",$lab_id)
                                ->get()->result();
